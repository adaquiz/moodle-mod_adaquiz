 ...
 */
function adaquiz_print_question_list($quiz, $pageurl, $allowdelete, $reordertool,
    ...
    $strremove = get_string('remove', 'adaquiz');
    ...
    $strsave = get_string('save', 'adaquiz');
    ...
    if (!empty($questionIds)){
        ...
        $questions = $DB->get_records_sql("SELECT q.*, qc.contextid, an.grade as maxmark
                              ...
                              JOIN {adaquiz_node} an ON an.question = q.id
                             WHERE q.id $usql AND an.adaquiz = ?", $params);
    }else{
        $questions = array();
    }

    $layout = quiz_clean_layout($questionIds);
    ...
    foreach ($order as $count => $qnum) {
        ...
        if ($qnum && !array_key_exists($qnum, $questions)) {
            ...
            $fakequestion->name = get_string('missingquestion', 'adaquiz');
        ...
        if ($qnum != 0 || ($qnum == 0 && !$pageopen)) {
            ...
            if ($qnum == 0  && $count < $questiontotalcount) {
                ...
                print_string('nonodesonquiz', 'adaquiz');
                echo '</div>';
            }
    ...
    <div class="questioncontainer <?php echo $question->qtype; ?>">
        <div class="qnum">
                ...
                if ($question->length == 0) {
                    $qnodisplay = get_string('infoshort', 'adaquiz');
                } 
        ...
        <div class="content">
            <div class="questioncontrols">
                ...
                if (!in_array($question->qtype, array('description', 'missingtype'))) {
                    ?>
        ...
        <input type="submit" class="pointssubmitbutton" value="<?php echo $strsave; ?>" />
                    ...
                    if ($question->qtype == 'random') {
                        echo '<a href="' . $questionurl->out() .
                                ...
                                get_string("configurerandomquestion", "adaquiz") . '</a>';
                    }
            ...
            <div class="questioncontentcontainer">
                ...
                if ($question->qtype == 'random') { // It is a random question.
                    ...
                    adaquiz_print_singlequestion($question, $returnurl, $quiz, $hasattempts);
                }
                ?>
            </div>
        </div>
...
function quiz_print_pagecontrols($quiz, $pageurl, $page, $hasattempts,
    ...
    if ($canaddquestion) {
        ...
        create_new_question_button($defaultcategoryid, $newquestionparams,
                get_string('addaquestion', 'adaquiz'),
                get_string('createquestionandadd', 'adaquiz'), $hasattempts);
    }
        ...
        <form class="randomquestionform" action="<?php echo $CFG->wwwroot;
            ...
            <div>
                ...
                <input type="submit" id="addrandomdialoglaunch_<?php
                        ...
                        echo get_string('addarandomquestion', 'adaquiz'); ?>" <?php
                        echo " $disabled"; ?> />
            </div>
        </form>
    </div>
 ...
 */
function adaquiz_print_singlequestion($question, $returnurl, $quiz, $hasattempts) {
    ...
    if (!$hasattempts){
        echo '<span class="adaquiz">' . adaquiz_question_modify_button($quiz, $question) . '</span>';        
    }
    echo "</div>\n";
}
/**
...
function quiz_print_randomquestion(&$question, &$pageurl, &$quiz, $quiz_qbanktool) {
    ...
    echo ' ' . get_string('randomfromcategory', 'adaquiz') . '</div>';

    $a = new stdClass();
    $a->arrow = $OUTPUT->rarrow();
    $strshowcategorycontents = get_string('showcategorycontents', 'adaquiz', $a);
    ...
    if ($questioncount == 0) {
        ...
        echo get_string('addnewquestionsqbank', 'adaquiz', $a);
...
function quiz_print_singlequestion_reordertool($question, $returnurl, $quiz) {
    ...
    echo '</label>';
    echo '<span class="questionpreview">'; // . quiz_question_action_icons($quiz, $quiz->cmid, $question, $returnurl) . '</span>';
    echo "</div>\n";
}
...
function print_random_option_icon($question) {
    ...
    if (!empty($question->questiontext)) {
        $icon = 'withsubcat';
        $tooltip = get_string('randomwithsubcat', 'adaquiz');
    } else {
        $icon = 'nosubcat';
        $tooltip = get_string('randomnosubcat', 'adaquiz');
    }
...
function quiz_question_tostring($question, $showicon = false,
    ...
    if ($showquestiontext) {
        ...
        if (!empty($questiontext)) {
            ...
            $result .= get_string('questiontextisempty', 'adaquiz');
            $result .= '</span>';
        }
        $result .= '</span>';
    }
...
function quiz_print_grading_form($quiz, $pageurl, $tabindex) {
    ...
    $a = '<input type="text" id="inputmaxgrade" name="maxgrade" size="' .
         ...
         . '" value="' . adaquiz_format_grade($quiz, $quiz->grade) . '" />';
 ...
 * @param object $quiz The quiz object of the quiz in question
 */
function adaquiz_print_status_bar($quiz) {
    global $CFG;

    $bits = array();

    $bits[] = html_writer::tag('span',
            get_string('totalmarksx', 'adaquiz', adaquiz_format_grade($quiz, $quiz->sumgrades)),
            array('class' => 'totalpoints'));

    $bits[] = html_writer::tag('span',
            get_string('numquestionsx', 'adaquiz', adaquiz_number_of_questions_in_quiz($quiz->questions)),
