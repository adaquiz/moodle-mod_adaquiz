...
abstract class adaquiz_attempts_report_table extends table_sql {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/adaquiz/review.php', array('attempt' => $attempt->attempt, 'cmid' => $this->options->cm->id)),
    ...
    public function col_timestart($attempt) {
        if ($attempt->attempt) {
            return userdate($attempt->timecreated, $this->strtimeformat);
        } else {
            return  '-';
        }
    }
    ...
    public function col_timefinish($attempt) {
        if ($attempt->attempt && $attempt->timemodified) {
            return userdate($attempt->timemodified, $this->strtimeformat);
        } else {
            return  '-';
        }
    }
    ...
    public function col_duration($attempt) {
        if ($attempt->timemodified) {
            return format_time($attempt->timemodified - $attempt->timecreated);
        } else {
            return '-';
        }
    }
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $output = html_writer::tag('span', html_writer::tag('span',
                ...
                array('class' => $class)), array('class' => 'que'));

        $url = new moodle_url('/mod/adaquiz/reviewquestion.php',
    ...
    public function base_sql($reportstudents) {
        ...
        $fields .= '
                ...
                quiza.timemodified,
                quiza.timecreated,
                CASE WHEN quiza.timemodified = 0 THEN null
                     WHEN quiza.timemodified > quiza.timecreated THEN quiza.timemodified - quiza.timecreated
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and adaquiz_attempt tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {adaquiz_attempt} quiza ON
                                    quiza.userid = u.id AND quiza.adaquiz = :quizid";
        ...
        if ($this->options->states) {
            foreach ($this->options->states as $key => $value){
                if ($value == 'inprogress'){
                    $this->options->states[$key] = Attempt::STATE_ANSWERING;
                }else if ($value == 'finished'){
                    $this->options->states[$key] = Attempt::STATE_FINISHED;
                }
            }
