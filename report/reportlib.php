 ...
 */
function adaquiz_report_get_significant_questions($quiz) {
    global $DB;

    $questionids = adaquiz_questions_in_quiz($quiz->questions);
...
SELECT
    q.id,
    q.length,
    aqn.grade AS maxmark

FROM {question} q
JOIN {adaquiz_node} aqn ON aqn.question = q.id
 ...
 */
function adaquiz_report_qm_filter_select($quiz, $quizattemptsalias = 'quiza') {
    ...
    switch ($quiz->grademethod) {
        case QUIZ_GRADEHIGHEST :
            return "$quizattemptsalias.id = (
                    SELECT MIN(qa2.id)
                    FROM {adaquiz_attempt} qa2
                    WHERE qa2.adaquiz = $quizattemptsalias.adaquiz AND
                        qa2.userid = $quizattemptsalias.userid AND
                        COALESCE(qa2.sumgrades, 0) = (
                            SELECT MAX(COALESCE(qa3.sumgrades, 0))
                            FROM {adaquiz_attempt} qa3
                            WHERE qa3.adaquiz = $quizattemptsalias.adaquiz AND
...
function adaquiz_report_highlighting_grading_method($quiz, $qmsubselect, $qmfilter) {
    if ($quiz->attempts == 1) {
        ...
        return '<p>' . get_string('showinggradedandungraded', 'quiz_overview',
                '<span class="gradedattempt">' . adaquiz_get_grading_option_name($quiz->grademethod) .
                '</span>') . '</p>';
    }
}
...
function quiz_report_scale_summarks_as_percentage($rawmark, $quiz, $round = true) {
    ...
    if ($round) {
        $mark = adaquiz_format_grade($quiz, $mark);
    }
    return $mark . '%';
}
 ...
 */
function adaquiz_report_download_filename($report, $courseshortname, $quizname) {
 ...
 */
function adaquiz_no_questions_message($quiz, $cm, $context) {
    global $OUTPUT;
...
function quiz_report_should_show_grades($quiz, context $context) {
    if ($quiz->timemodified && time() > $quiz->timemodified) {
        $when = 'adaquiz';
    } else {
        $when = 'questions';
    }
    ...
    return adaquiz_has_grades($quiz) &&
