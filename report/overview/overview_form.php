...
/**
 * This file defines the setting form for the adaptive quiz overview report.
 *
...
/**
 * Adaptive quiz overview report settings form.
 */
...
class adaquiz_overview_settings_form extends mod_adaquiz_attempts_report_form {
    ...
    protected function other_attempt_fields(MoodleQuickForm $mform) {
        if (has_capability('mod/adaquiz:regrade', $this->_customdata['context'])) {
            $mform->addElement('advcheckbox', 'onlyregraded', get_string('reportshowonly', 'adaquiz'),
                    get_string('optonlyregradedattempts', 'adaquiz_overview'));
            $mform->disabledIf('onlyregraded', 'attempts', 'eq', adaquiz_attempts_report::ENROLLED_WITHOUT);
        }
    }

    protected function other_preference_fields(MoodleQuickForm $mform) {
        if (adaquiz_has_grades($this->_customdata['adaquiz'])) {
            $mform->addElement('selectyesno', 'slotmarks',
                    get_string('showdetailedmarks', 'adaquiz_overview'));
        } else {
