 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class adaquiz_attempts_report extends quiz_default_report {
    ...
    protected function init($mode, $formclass, $quiz, $cm, $course) {
        ...
        $this->qmsubselect = adaquiz_report_qm_filter_select($quiz);
    ...
    protected function add_grade_columns($quiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            ...
            $headers[] = get_string('grade', 'adaquiz') . '/' .
                    adaquiz_format_grade($quiz, $quiz->grade);
        }
    ...
    protected function delete_selected_attempts($quiz, $cm, $attemptids, $allowed) {
        ...
        foreach ($attemptids as $attemptid) {
            ...
            $attempt = Attempt::getAttemptById($quiz, $attemptid);
            if (!$attempt || $attempt->adaquiz != $quiz->id || $attempt->preview != 0) {
