...
class quiz {
    ...
    public function __construct($quiz, $cm, $course, $getcontext = true) {
        ...
        $questionids = adaquiz_questions_in_quiz($this->quiz->questions);
        
        if ($questionids) {
            $this->questionids = explode(',', adaquiz_questions_in_quiz($this->quiz->questions));
        } else {
    ...
    public static function create($quizid, $userid) {
        ...
        $cm = get_coursemodule_from_instance('adaquiz', $quiz->id, $course->id, false, MUST_EXIST);

        $quizQuestions = $DB->get_records('adaquiz_node', array('adaquiz' => $quiz->id), 'position ASC', 'question');
        $quiz->questions = implode(',', array_keys($quizQuestions));
     ...
     * @return quiz_attempt the new adaquiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new adaquiz_attempt($attemptdata, $this->quiz, $this->cm, $this->course);
    }
    ...
    public function preload_questions() {
        ...
        $this->questions = question_preload_questions($this->questionids,
                'an.grade AS maxmark, an.id AS instance',
                '{adaquiz_node} an ON an.adaquiz = :adaquizid AND q.id = an.question',
                array('adaquizid' => $this->quiz->id));
    }
     ...
     */
    public function attempt_url($attemptid, $page = 0, $id = null, $nextnode = null, $rewnode = null) {
        global $CFG;
    ...
    public function confirm_start_attempt_message($unfinished) {
        ...
        if ($this->quiz->timelimit && $this->quiz->attempts) {
            return get_string('confirmstartattempttimelimit', 'adaquiz', $this->quiz->attempts);
        } else if ($this->quiz->timelimit) {
            return get_string('confirmstarttimelimit', 'adaquiz');
        } else if ($this->quiz->attempts) {
            return get_string('confirmstartattemptlimit', 'adaquiz', $this->quiz->attempts);
        }
...
class adaquiz_attempt {
    ...
    protected static function create_helper($conditions) {
        ...
        $attempt = $DB->get_record('adaquiz_attempt', $conditions, '*', MUST_EXIST);
        $quiz = quiz_access_manager::load_quiz_and_settings($attempt->adaquiz);
        $course = $DB->get_record('course', array('id' => $quiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('adaquiz', $quiz->id, $course->id, false, MUST_EXIST);

        // Update quiz with override information.
        $quiz = adaquiz_update_effective_access($quiz, $attempt->userid);

        $qa = new adaquiz_attempt($attempt, $quiz, $cm, $course);
        $count = $DB->count_records(NodeAttempt::TABLE, array('attempt' => $conditions['id']));
        ...
        /*foreach($slots as $key => $value){
            array_push($pagelayout, array($value));
        }*/
        ...
        $qa->pagelayout = $pagelayout;
        
        return $qa;
    }

    /**
     * Static function to create a new adaquiz_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new adaquiz_attempt object given a usage id.
     *
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                ...
                if ($question->length > 0) {
                    ...
                    $question->_number = get_string('infoshort', 'adaquiz');
                }
                $question->_page = $page;
            }
        }
    ...
    public function get_overall_feedback($grade) {
        return adaquiz_feedback_for_grade($grade, $this->get_quiz(),
    ...
    public function get_question_mark($slot) {
        return adaquiz_format_question_grade($this->get_quiz(), $this->quba->get_question_mark($slot));
    }
     ...
     */
    public function attempt_url($slot = null, $page = -1, $thispage = -1, $nextnode = null, $rewnode = null) {
     ...
     */
    public function summary_url($cmid = 0) {
        return new moodle_url('/mod/adaquiz/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $cmid));
    }
     ...
     */
    public function render_question($slot, $reviewing, $thispageurl = null, $qnum = -1) {
        $options = $this->get_display_options_with_edit_link($reviewing, $slot, $thispageurl);
        if ($qnum != -1){
            return $this->quba->render_question($slot, $options, $qnum);    
        }else{
    ...
    public function get_navigation_panel(mod_adaquiz_renderer $output,
             $panelclass, $page, $showall = false, $route = -1) {
        ...
        $bc->content = $output->navigation_panel($panel, $route);
        return $bc;
    }
    ...
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = adaquiz_get_user_attempts($this->get_quiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false) {
        ...
        if ($this->attempt->state == Attempt::STATE_FINISHED) {
            $this->attempt->sumgrades = $this->quba->get_total_mark();
        }
        /*if ($becomingoverdue) {
            $this->process_going_overdue($timestamp, true);
        } else {
            $DB->update_record('adaquiz_attempt', $this->attempt);
        }*/

        $DB->update_record('adaquiz_attempt', $this->attempt);
        
        if (!$this->is_preview() && $this->attempt->state == Attempt::STATE_FINISHED) {
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        $this->attempt->sumgrades = $this->quba->get_total_mark();
        $this->attempt->state = Attempt::STATE_FINISHED;
        $DB->update_record('adaquiz_attempt', $this->attempt);
...
abstract class quiz_nav_panel_base {
    ...
    public function get_question_buttons() {
        $buttons = array();
        $test = $this->attemptobj->get_slots();
...
class quiz_attempt_nav_panel extends quiz_nav_panel_base {
    public function get_question_url($slot) {
        if ($this->attemptobj->can_navigate_to($slot)) {
        	//Add node parameter for the last page
        	if ($this->attemptobj->is_last_page($slot-1)) {
        		$attemptid = $this->attemptobj->get_attemptid();
            ...
            return $this->attemptobj->attempt_url($slot, -1, $this->page, $nextnode);
        } else {
            return null;
        }
    }
    ...
    public function render_end_bits(mod_adaquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url($this->attemptobj->get_cmid()),
