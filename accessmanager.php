...
class quiz_access_manager {
    ...
    public static function load_quiz_and_settings($quizid) {
        ...
        $sql = 'SELECT quiz.* FROM {adaquiz} quiz WHERE quiz.id = :quizid';
        $params = array('quizid' => $quizid);
        $quiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        $questions = Node::getAllNodes($quiz->id);
        $q = Array();
        $qsumgrades = 0;
        foreach ($questions as $key => $value){
            $qsumgrades += $value->grade;
            array_push($q, $value->id);
        }
        $questionid = implode(',', $q);
        $quiz->questions = $questionid;
        $quiz->decimalpoints = 2;
        $quiz->sumgrades = $qsumgrades;
